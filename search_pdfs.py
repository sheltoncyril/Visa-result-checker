...
def search_pdfs(folder, filepaths: List[Path], search_ids: List) -> pd.DataFrame:
    ...
    if cache_df_file.exists() and cache_df_file.stat().st_size > 0:
        cache_df = pd.read_csv(cache_df_file)
    else:
    ...
    for filepath in filepaths:
        ...
        c += 1
        print(f"File {c}/{len(filepaths)} complete.", end="\r")
    cache_df.drop_duplicates(subset=["ID"], keep="last")
    cache_df.to_csv(cache_df_file)
    print(f"Processed: {processed}, Cached: {cached}, Total rows: {len(cache_df)}")
